...
public class PrepareJob {
	...
	public static int queryEnCollect() throws SQLException{
		...
		String preHql = " insert overwrite table query_en partition(day='"+Constants.getToday()+"') ";
		String hql = preHql+" select reqid,uid,pid,tab.col1,nation,adid,pv,impr,sv,click from log_merge lateral view qs(query,':') tab as col1 " +
				"where day ='"+Constants.getToday()+"' and array_contains(array('in','us','pk','ph','gb','au','za','lk','ca','sg','sg','nz','ie','ng','gh','cm'),nation) and query is not null";
